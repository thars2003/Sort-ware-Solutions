...
class A4988StepperMotor:
    ...
    def rotate_degrees(self, degrees, rpm=30, clockwise=True):
        """
        ...
        print(f"Rotating {degrees} degrees at {rpm} RPM ({'CW' if clockwise else 'CCW'}), step delay: {delay:.4f}s")
        self.step(steps, delay, clockwise)
...
def main():
    ...
    try:
        ...
        print("\n--- Example 4: 200 steps with 0.005s delay ---")
        motor.step(steps=200, delay=0.005, clockwise=True)
        time.sleep(1)
        
        # Example 5: Faster rotation - 1 revolution at 40 RPM
        print("\n--- Example 5: 1 revolution at 40 RPM ---")
